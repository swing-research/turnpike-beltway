...
bool isDistanceSubsetOf(vector<int> distanceList,vector<int> L, int err_range) {
		...
		for (int i=0;i<distanceList.size();i++){
			...
			for (int j=0; j<L.size(); j++) {
			    int distance_compute = abs(uu-L[j]);
			    if ( distance_compute <= err_range) {
			        
			        if (index_min==-1) {
			            index_min = j;
			            distance_min = distance_compute;
			        } else {
			            if (distance_compute<distance_min) {
			                index_min = j;
			                distance_min = distance_compute;
			            }
			        }
...
vector<int> removePartialDigests(vector<int> delta, vector<int> &L, int err_range){
...
void print_to_output (vector< vector<int> > solutions, char* fileName_output)
{
    writefile_output.open(fileName_output, ios_base::trunc);
    for (int i=0; i<solutions.size(); i++) {
        vector<int> solution_one = solutions[i];
        for(int j=0;j<solution_one.size();j++) {
            writefile_output<<solution_one[j]<<" ";
        }
        writefile_output<<endl;
    }
    writefile_output.close();
...
vector<Snapshot> divide(vector<Snapshot> snapshot_stack)
{
	...
	for(int i=0;i<snapshot_stack.size();i++)
	{
			...
			tst1 = isDistanceSubsetOf(deltaList_1, currentSnapshot.L1, err_range_set);
			tst2 = isDistanceSubsetOf(deltaList_2, currentSnapshot.L1, err_range_set);
			...
			else if (tst1 && tst2)
			{
				...
				vector<int> removed = removePartialDigests(deltaList_1, currentSnapshot.L1, err_range_set);
				...
				vector<int> removed_1 = removePartialDigests(deltaList_2, currentSnapshot.L1, err_range_set);
				if(!compare_element_to_vector(result,currentSnapshot))
					result.push_back(currentSnapshot);
			}
			else if (tst1)
...
void place_BBb2(vector<int> L, vector<int> X){
	...
	cout<<"Snapshot size: "<<snapshot_stack.size()<<endl;
	/*
	while(!snapshot_stack.empty())
	{
		snapshot_stack = divide(snapshot_stack);
...
vector< vector<int> > partialDigestProblem(vector<int> L){
	...
	
{
	...
	else if(d_selection == 1)
	{
		...
		//main function
		int start_s=clock();
		vector< vector<int> > solutions_out = partialDigestProblem(rand_instance);
		int stop_s=clock();
	...
	else
	{
	    ...
	    std::istringstream alg_idx_str(argv[3]);	
        if (!(alg_idx_str >> algorithm_idx)) {
            std::cerr << "Invalid number: " << argv[3] << '\n';
            abort();
        }
        
        std::istringstream err_range_str(argv[4]);
        if (!(err_range_str >> err_range_set)) {
            std::cerr<< "Incorrect error range: " << argv[4]<< '\n';
            abort();
        }
        ...
        if (algorithm_idx==0) {
            ...
            strcpy(Algorithm,"BBd");
            cout<<"Using BBd."<<endl;
        } else if (algorithm_idx==1) {
            BBb_ = true;
            strcpy(Algorithm,"BBb");
            cout<<"Using BBb."<<endl;
        } else if (algorithm_idx==2) {
            BBb2_ = true;
            strcpy(Algorithm,"BBb2");
            cout<<"Using BBb2."<<endl;
        } else {
            std::cerr<<"Unrecognized algorithm choice. Please select a number from 0 to 2.\n";
            abort();
        }
		...
		print_to_output(solutions_out, argv[2]);
