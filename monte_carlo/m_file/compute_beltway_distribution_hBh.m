function foo = compute_beltway_distribution_hBh(LN_num, dist_unit, num_mc, trial_num, output)
...
loc = 0:K-1;
loc_mark = zeros(K,1);  % the candidate locations
for (i=1:K)
...
for (i=1:num_mc)
    ...
    val_sim_tmp = 0;
    for (j=1:K)
        val_sim_tmp = val_sim_tmp + (2*sum(h_s_norm(1:(K-j+1)).*h_s_norm(j:end)) + 2*sum(h_s_norm(end-j+1:end).*h_s_norm(1:j)))^2;
    end 
