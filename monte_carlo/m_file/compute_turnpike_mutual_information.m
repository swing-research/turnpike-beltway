function foo = estimate_turnpike_mutual_information(LN_num, dist_unit, total_trial_num, output)
...
d_count_all = zeros(round(dist_max/dist_unit)+1+ceil(3*n_std),1);
...
for (trial_num = (1:total_trial_num))
    ...
    for (i=1:Ke)
        if (x_loc(i)>0)
            y_prob_tmp = zeros(Ke, 1); 
            y_prob_tmp(1:Ke-i+1) = x_loc(i:end);
            if (i>1)
                y_prob_tmp(1:i-1) = y_prob_tmp(1:i-1) + x_loc(i-1:-1:1);
            end
            ...
            if (y_prob_tmp_sum>0)
                ...
                %fprintf('%d\t%d\n', sum(y_prob_tmp), sum(-y_prob_tmp.*log(y_prob_tmp)))
            end
        end 
    end
