function foo = compute_turnpike_distribution_hBh(LN_num, dist_unit, num_mc, trial_num, output)
...
for (i=1:num_mc)
    ...
    val_sim_tmp = 0;
    for (j=1:K)
        val_sim_tmp = val_sim_tmp + (2*sum(h_s_norm(1:(K-j+1)).*h_s_norm(j:end)))^2;
    end 
