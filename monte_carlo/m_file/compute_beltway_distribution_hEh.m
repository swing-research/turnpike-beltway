function foo = compute_beltway_distribution_hEh(LN_num, dist_unit, num_mc, trial_num, output)
...
for (i=1:N)
    for (j=i:N)
        ...
        d_seq = [d_seq K-abs(x_ori(i)-x_ori(j))];
    end 
end
...
loc_mark = zeros(K,1);
for (i=1:K)
    dist_tmp_1 = abs(loc(i)-x_ori(1));
    dist_tmp_2 = abs(loc(i)-x_ori(2));
    dist_tmp_3 = K-abs(loc(i)-x_ori(1));
    dist_tmp_4 = K-abs(loc(i)-x_ori(2));
    if ( (min(abs(d_uq-dist_tmp_1))==0) && (min(abs(d_uq-dist_tmp_2))==0) )
        if ( (min(abs(d_uq-dist_tmp_3))==0) && (min(abs(d_uq-dist_tmp_4))==0) )
            loc_mark(i)=1;
        end
    end
end
...
for (i=1:K)
    ...
    E_seq_tmp_3 = zeros(K,1);
    E_seq_tmp_4 = zeros(K,1);

    E_seq_tmp_1(1:K-i+1) = x_loc(i:end);
    E_seq_tmp_2(i:end) = x_loc(1:K-i+1);
    E_seq_tmp_3(end-i+1:end) = x_loc(1:i);
    E_seq_tmp_4(1:i) = x_loc(end-i+1:end);
    E_seq_tmp = E_seq_tmp_1+E_seq_tmp_2+E_seq_tmp_3+E_seq_tmp_4;
